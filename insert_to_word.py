...
try:
    ...
    PDF_SUPPORT = False
    print("Warning: PyMuPDF tidak terinstall. PDF/Word tidak akan diproses.")
...
def convert_pdf_to_image(pdf_path, output_image_path):
    """Convert PDF ke image (halaman pertama) menggunakan PyMuPDF"""
    if not PDF_SUPPORT:
        return False
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "  python insert_to_word.py '/path/to/dokumentasi penelitian' output.docx"
        )
        sys.exit(1)
